...
function handleNotify(data, res) {
  ...
  const safeTERM_ID = escapeForAppleScript(TERM_ID);
  if (TERM_PROGRAM === "ghostty" && TERM_ID) {
    focusTerminalScript = `
      tell application "Ghostty"
        activate
        focus terminal id "${safeTERM_ID}"
      end tell`;
  } else if (
    ...
    focusTerminalScript = `
      tell application "Terminal"
        set index of (first window whose id is ${safeTERM_ID}) to 1
        activate
      end tell`;
        ...
        repeat with w in windows
          repeat with t in tabs of w
            repeat with s in sessions of t
              if unique id of s is "${safeTERM_ID}" then
                select s
                select t
                select w
                activate
  ...
  const osaScript = `
    say "${escapeForAppleScript(announcement)}"
    ${dialogScript}
  `;
...
function startCallbackProxy(port, runtime, containerId) {
  const proxy = http.createServer((req, res) => {
    ...
    try {
      // Sanitize req.url to prevent shell injection — only allow URL-safe chars
      const safeUrl = req.url.replace(
        /[^a-zA-Z0-9\-._~:/?#[\]@!$&'()*+,;=%]/g,
        "",
      );
      execSync(
        `${runtime} exec ${containerId} curl -sf "http://localhost:${port}${safeUrl}"`,
