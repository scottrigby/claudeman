...
//   Line 1: eventType (complete, question, idle, info, open-url)
...
const server = net.createServer((socket) => {
  ...
  socket.on("end", () => {
    ...
    // Handle open-url event: open the URL in the host's default browser.
    // For OAuth URLs with a localhost callback, proxy the callback into the container.
    // Extended payload: open-url\nTERM\nTERM_ID\nURL\nCALLBACK_PORT\nRUNTIME\nCONTAINER_ID
    if (eventType === "open-url") {
      // Parse extended fields from remaining lines
      const msgLines = RECEIVED_MESSAGE.split("\n");
      const openUrl = (msgLines[0] || "").trim();
      const callbackPort = (msgLines[1] || "").trim();
      const containerRuntime = (msgLines[2] || "").trim();
      const containerId = (msgLines[3] || "").trim();
      ...
      // If there's a callback port, set up a proxy to forward the OAuth
      // callback from the host's localhost into the container
...
function startCallbackProxy(port, runtime, containerId) {
  const proxy = http.createServer((req, res) => {
    ...
    try {
      // Forward the callback into the container
      execSync(
      ...
      // The callback may "fail" (non-200) but still work — Claude Code
      // receives the code and completes auth regardless
      process.stderr.write(
