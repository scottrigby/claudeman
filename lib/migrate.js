...
// Load hooks.json profile replacements.
// Returns Map: v1FileName -> { profile, description }
export function loadProfileReplacements(hooksJsonPath) {
  const { profileReplacements } = JSON.parse(
    fs.readFileSync(hooksJsonPath, "utf8"),
  );
  return new Map(Object.entries(profileReplacements || {}));
}

// Load hooks.json conversion rules and return a toV2Command(cmd) lookup function.
// hooksJsonPath: absolute path to hooks.json
// Conversions is an object mapping exact v1 command strings to v2 equivalents.
export function loadConversions(hooksJsonPath) {
  const { conversions } = JSON.parse(fs.readFileSync(hooksJsonPath, "utf8"));
  const map = new Map(Object.entries(conversions));
  return (cmd) => map.get(cmd) || null;
}
